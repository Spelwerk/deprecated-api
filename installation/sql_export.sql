...
CREATE TABLE IF NOT EXISTS `spelwerk`.`world` (
  ...
  `roll_expertisetype_id` INT NOT NULL,
  ...
  `max_characteristic` INT NOT NULL,
  `max_supernatural` INT NOT NULL,
  `max_skill` INT NOT NULL,
  `max_expertise` INT NOT NULL,
  `max_milestone` INT NOT NULL,
  `max_relationship` INT NOT NULL,
  `max_timeline_upbringing` INT NOT NULL,
  `max_timeline_flexible` INT NOT NULL,
  `created` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `deleted` DATETIME NULL,
  `updated` DATETIME NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_setting_attributetype2_idx` (`skill_attributetype_id` ASC),
  INDEX `fk_world_expertisetype2_idx` (`roll_expertisetype_id` ASC),
  INDEX `fk_world_expertisetype3_idx` (`attribute_expertisetype_id` ASC),
  UNIQUE INDEX `hash_UNIQUE` (`hash` ASC),
  CONSTRAINT `fk_setting_attributetype2`
    FOREIGN KEY (`skill_attributetype_id`)
    REFERENCES `spelwerk`.`attributetype` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_world_expertisetype2`
    FOREIGN KEY (`roll_expertisetype_id`)
    REFERENCES `spelwerk`.`expertisetype` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_world_expertisetype3`
